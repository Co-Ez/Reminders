...
//DONT INCLUDE THIS COMMENT OR THE COMMENTS ABOVE, USE COMMON SENSE, THERE ARE ALSO COMMENTS THAT NEED REMOVING BELOW
...
if ($_SERVER["REQUEST_METHOD"] == "POST") { //checks if the form has been submitted
    ...
    $dob = strip_tags($_POST['new_dob']); //short for date of birth
    ...
    //line 72-82 is checking if the person is over 18 or not
    if (is_string($dob)) {
        $dob_comf = strtotime($dob);
    }

    if (time() - $dob_comf < 18 * 31536000) {
        $dob_comf = false;
    }
    else {
        $dob_comf = true;
    ...
    try { //trys to insert the values into the table, prevents errors crashing the program
        if ($dob_comf == false) { //checking if the person is 18 years old or not, if not, it creates an error message
            throw new Exception("Not 18 years or Older");
        }
        ...
        $stmt = $con->prepare("INSERT INTO userinformation (email, first_name, last_name, password, dob) VALUES (?, ?, ?, ?, ?)"); //prepares the statement to insert the values into the table
        $stmt->bind_param("sssss", $email, $first_name, $last_name, $password, $dob); //binds all the parameters with the correct values
        }
